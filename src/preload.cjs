...
contextBridge.exposeInMainWorld('pb', {
  ...
  upCheck: () => ipcRenderer.invoke('up:check'),
  upDownload: () => ipcRenderer.invoke('up:download'),
  upSetFolder: (id) => ipcRenderer.invoke('up:setFolder', id),
  upGetFolder: () => ipcRenderer.invoke('up:getFolder'),
  onUpdateAvailable: (cb) => ipcRenderer.on('pb:updateAvailable', (_e, p) => cb(p)),
